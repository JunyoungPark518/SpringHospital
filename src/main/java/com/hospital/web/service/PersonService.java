...
public class PersonService {
	...
	public Admin getAdmin(Map<?,?> paramMap)throws Exception{
		IGetService service= (map)->mapper.findAdmin(map); 
		return (Admin) service.execute(paramMap);
	}
	public Map<?,?> postPatient(Object target) throws Exception{
		Map<String,String>map=new HashMap<>();
		IPostService service=(patient)->mapper.insertPatient(patient);
		map.put("result", (service.execute(target)==1)?"success":"fail");
		return map;
	}
	public Map<?,?> postDoctor(Object target) throws Exception{
		Map<String,String>map=new HashMap<>();
		IPostService service=(doctor)->mapper.insertDoctor(doctor);
		map.put("result", (service.execute(target)==1)?"success":"fail");
		return map;
	}
	public Map<?,?> postNurse(Object target)throws Exception{
		Map<String,String>map=new HashMap<>();
		IPostService service=(nurse)->mapper.insertNurse(nurse);
		map.put("result", (service.execute(target)==1)?"success":"fail");
		return map;
	}
	public Map<?,?> postAdmin(Object target)throws Exception{
		Map<String,String>map=new HashMap<>();
		IPostService service=(admin)->mapper.insertAdmin(admin);
		map.put("result", (service.execute(target)==1)?"success":"fail");
		return map;
	}
	...
	public Integer exist(Map<?,?> map) throws Exception{
		IGetService service1 = (a) -> mapper.exist(a);
		return (Integer) service1.execute(map);
	}
